...
> **過期項目（2026-09-02）**：原文件要求的 12 筆 B2C 展示商品已不再是 MVP 需求；現行展示資料以 `supabase/seed.sql` 的 6 筆為準。以下 12 筆內容與審查結論僅保留作歷史追溯。

## 1. 已具備的內容

原文件曾涵蓋 12 筆商品、6 個分類、4 組標籤、商品描述、AND 篩選、無結果、缺漏欄位、ProductCard、ProductDetail、事件規格及 18 個驗收案例；12 筆需求現已過期，僅作 UI／UX 與前端歷史參考。
...
- 原文件設計 12 筆商品的需求已過期；現行 `supabase/seed.sql` 採 6 筆展示資料，slug、品牌、分類與標籤內容以現行 seed 為準。
- 文件內沒有可直接執行的 12 筆 SQL seed，也沒有 UUID／關聯表資料。
- 不再評估「取代／並存 12 筆」；12 筆只作歷史測試案例，現行功能不以 12 筆作為驗收數量。
...
1. 12 筆需求已結案為過期；現行 seed 與驗收以 6 筆展示資料為準。
2. 統一 ProductCard 顯示欄位與分類名稱／slug。
3. 決定 PR #1 的 schema 草案是否採用，修正後再合併。
4. 若未來需要擴充，再另立商品資料需求，不回復本文件的 12 筆數量要求。
