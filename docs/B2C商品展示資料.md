...
| 測試面向 | 說明 | 對應商品 |
|---|---|---|
| 不同分類 | 涵蓋魚類、蝦蟹類、貝類、軟體類、肉類、調理食品類共 6 個分類 | 全部 12 筆 |
...
標籤定義由開發團隊預先建立（不開放後台自由新增），本表僅列出本批 12 筆展示商品實際套用的標籤值，符合 PRD 3.2／6.1 標籤治理原則。
...
- 本批資料共 12 筆，用於驗證 MVP 資料結構、篩選與搜尋邏輯，非正式上線的完整商品清單（正式規模為 2000+ 品項，依 PRD 3.2／6.3 規劃於 MVP 後續分批建置）。
...
檢查結果：本文件第 0～5 章與已套用資料庫 schema／種子資料之間存在下列落差。原則上以資料庫 schema 為準設計第 7～10 章型別與規則，但**不回頭改寫第 0～5 章已完成的 12 筆商品資料**，僅在此標記，供團隊決定後續調整方向。
...
| `state.status === "not_found"` | 顯示「找不到這項商品」（暫定文案，待團隊確認，見 C11），提供返回商品列表的連結；設定對應的 HTTP 狀態（Next.js 內建 `notFound()`），並確保頁面 `noindex` |
| `state.status === "inactive"` | 僅 Admin 情境；顯示「已下架」徽章（暫定文案，見 C11）＋商品完整資料，供後台檢視 |
...
| 10 | 商品不存在 | 任意不存在的 slug，例如 `/products/does-not-exist` | 進入該網址 | `state.status = "not_found"`，顯示「找不到這項商品」（暫定文案，見 C11），設定 404，`noindex` |
