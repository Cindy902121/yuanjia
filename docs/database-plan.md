...
- 每項商品最多 1 張封面圖與 5 張細節圖；每張圖有 `cover`／`detail` 角色、`alt` 文字與排序。後台可上傳、替換、刪除、排序與編輯 `alt`；刪除時同步刪除資料列與 Storage 物件，商品可暫時沒有圖片並顯示「尚無圖片」。清理失敗會保留警告並提供重試。
...
> 本節的 `[x]` 代表需求與驗收規格已確認；本次已完成商品 CRUD、圖片、CSV 批量匯入、前綴規則、規格選項、B2B 管理頁與分析篩選 API，以及對應 Storage／migration。
