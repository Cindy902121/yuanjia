...
- [x] 事件資料不包含姓名、電話、Email、IP、瀏覽器指紋或 User-Agent；完整客戶代碼與 company_id 僅由 server-side 安全契約保存於原始 B2B 事件快照，不進報表／CSV。
- [x] 分析篩選項：日期、級距、通路、產品、分類、品牌、事件名稱、篩選類型與 Finder 題目。
...
- 規則只由 migration／seed 建立與更新；Admin 只讀取規則來顯示級距／通路，不提供前綴規則管理頁籤或 CRUD API。規則異動不得回寫歷史事件，舊事件與詢價保留舊快照。
