# 元家企業／宅鮮配整合網站 MVP v3.1

## 最終路由與權限規格

- 文件日期：2026-09-09
- 依據：PRD v3.1、FDD v4.1 與逐題確認結果
- 適用：產品、開發、測試與驗收

## 文件目的

本文件固定 MVP 的頁面路由、角色權限、登入分流、SEO 索引、管理後台與 API 權限。未列出的成功狀態、商品詳情狀態與浮動工具均依 PRD 整合於既有頁面，不額外建立路由。

## 1. 角色定義

| 角色 | 定義 |
|---|---|
| 未登入 | 公開訪客，可使用 B2C 商品、購物車與展示結帳。 |
| B2C 會員 | 使用 Email／密碼或 Google 帳號登入，可進入 `/user`。 |
| B2B 公司使用者 | 使用客戶代碼與公司共用密碼登入；詢價與行為資料以 `company_id` 歸屬。 |
| 管理者 | 使用專屬帳號密碼從 `/login` 登入；登入頁不顯示管理員選項。 |

> 身分及權限必須由伺服器判斷；前端不得自行指定角色、`company_id`、客戶級距或通路。

## 2. 最終頁面路由與權限表

| 路由 | 用途 | 未登入 | B2C 會員 | B2B 公司使用者 | 管理者 | SEO／Sitemap |
|---|---|---|---|---|---|---|
| `/` | B2C 首頁 | 允許 | 允許 | 導向 `/business`，再進入型錄 | 允許瀏覽 | 可索引／加入 |
| `/about` | 品牌、企業、食安、品質與產地 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/faq` | 常見問題與使用說明 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/media` | 媒體與品牌內容 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/media/[slug]` | 媒體報導深度詳情 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/news` | 元家第一手最新消息 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/news/[slug]` | 元家消息詳情 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/products` | B2C 商品列表、搜尋與篩選 | 允許 | 允許 | 伺服器阻擋並導向 `/business` | 允許瀏覽 | 可索引／加入 |
| `/products/categories/[slug]` | B2C 獨立分類頁 | 允許 | 允許 | 伺服器阻擋並導向 `/business` | 允許瀏覽 | 可索引／加入 |
| `/products/tags/[slug]` | B2C 標籤商品列表 | 允許 | 允許 | 伺服器阻擋並導向 `/business` | 允許瀏覽 | 可索引；符合條件才加入[^2] |
| `/products/[slug]` | B2C 商品詳情 | 允許 | 允許 | 伺服器阻擋並導向 `/business` | 允許瀏覽 | 可索引／加入 |
| `/cart` | B2C 購物車 | 完整操作 | 完整操作 | 伺服器阻擋並導向 `/business` | 僅查看，不可修改 | `noindex`／不加入 |
| `/checkout` | 建立展示用模擬訂單 | 可建立 | 可建立 | 伺服器阻擋並導向 `/business` | 僅查看，不可建立 | `noindex`／不加入 |
| `/business/lead` | B2C 新客企業合作展示表單 | 允許 | 允許 | 可直接進入；導覽不顯示入口 | 允許 | 可索引／加入 |
| `/login` | B2C、B2B、管理者統一登入 | 顯示登入表單 | 導向 `/` | 導向 `/business/catalog` | 導向 `/admin` | `noindex`／不加入 |
| `/reset-password` | Email 密碼重設申請與連結後設定新密碼 | 顯示申請表單；有效連結可設定新密碼 | 顯示申請表單；有效連結可設定新密碼 | 顯示申請表單；不取代目前密碼驗證流程 | 顯示申請表單；有效連結可設定新密碼 | `noindex`／不加入 |
| `/signup` | B2C 新客建立帳號 | 顯示註冊表單 | 可建立帳號 | 公開頁面，不影響 B2B session | 公開頁面，不影響 Admin session | `noindex`／不加入 |
| `/user` | B2C 會員中心 | 導向 `/login` | 允許 | 登出確認[^1] | 導向 `/admin` | `noindex`／不加入 |
| `/business` | B2B 登入後內容首頁 | 導向 `/login` | 導回 `/` | 允許；顯示 B2B 內容首頁 | 導向 `/admin` | `noindex`／不加入 |
| `/business/about` | B2B 品牌故事入口 | 導向 `/login` | 導回 `/` | 轉至 `/business/about/company` | 導向 `/admin` | `noindex`／不加入 |
| `/business/about/[slug]` | B2B 品牌故事內容 | 導向 `/login` | 導回 `/` | 允許 | 導向 `/admin` | `noindex`／不加入 |
| `/business/news` | B2B 企業消息入口 | 導向 `/login` | 導回 `/` | 轉至 `/business/news/activities` | 導向 `/admin` | `noindex`／不加入 |
| `/business/news/{activities,offers,yuanjia}` | B2B 企業消息分類 | 導向 `/login` | 導回 `/` | 允許 | 導向 `/admin` | `noindex`／不加入 |
| `/business/news/article/[slug]` | B2B 企業消息詳情 | 導向 `/login` | 導回 `/` | 允許 | 導向 `/admin` | `noindex`／不加入 |
| `/business/catalog` | B2B 私有型錄 | 導向 `/login` | 提示後導向 `/` | 允許 | 導向 `/admin/business` 商品頁籤 | `noindex`／不加入 |
| `/business/product-finder` | B2B 固定需求篩選器 | 導向 `/login` | 提示後導向 `/` | 允許 | 導向 `/admin/business` 篩選器頁籤 | `noindex`／不加入 |
| `/business/rfq` | 詢價籃與過往詢價紀錄 | 導向 `/login` | 提示後導向 `/` | 僅同公司資料 | 導向 `/admin/business` 詢價頁籤 | `noindex`／不加入 |
| `/admin` | 管理後台 | 導向 `/login` | 導向 `/` | 導向 `/business` | 允許 | `noindex`／不加入 |
| `/admin/business` | B2B 管理功能 | 導向 `/login` | 導向 `/` | 導向 `/business` | 允許 | `noindex`／不加入 |

[^1]: B2B 誤入 B2C 商品、購物車或結帳路由時，由伺服器阻擋並導向 `/business`；直接呼叫 `/api/b2c/products` 回傳 `403`，不提供 B2B 進入 B2C 流程的例外。
[^2]: B2C 標籤頁只有在具備內容且至少有一項有效商品時，才加入 sitemap。

> `/business/homepage-preview`、`/business/prototype-home` 與 `/catalog-preview/*` 是非 MVP 預覽／原型路徑，不納入本表的正式角色權限；僅保留直接預覽用途並設定 `noindex`，不加入 sitemap。

## 3. 特殊路由內容

### `/user`

- 顯示 B2C 展示會員 Email 與登入狀態。
- 提供返回 B2C 首頁與登出。
- 不提供個資編輯、地址簿、密碼修改、訂單紀錄或 B2C 詢價紀錄。

### `/business/rfq`

- 同一路由以「目前詢價籃／過往詢價紀錄」頁籤切換。
- B2B 公司只能查看相同 `company_id` 的詢價紀錄。
- 同公司在不同瀏覽器使用共用帳密時，看到相同公司資料。
- 不得查看其他公司資料；不建立獨立詢價紀錄路由。

### B2B 內容頁

- `/business` 是登入後的 B2B 內容首頁，企業型錄仍由 `/business/catalog` 提供。
- `/business/about` 與 `/business/news` 是入口／分類導向頁；其子頁面只允許有效 B2B session 查看。
- B2B 內容頁不顯示價格、不加入 sitemap，並設定 `noindex`。

### `/admin`

- 單一頁面與頁籤：公司帳號、B2C 商品、既有標籤套用、前綴分類讀取、B2C 模擬訂單、B2B 分析。
- 提供前往 `/admin/business` 的入口。

### `/admin/business`

- 單一頁面與頁籤：B2B 商品新增修改、既有標籤套用移除、需求篩選器檢視操作、所有公司詢價管理。
- 管理者可操作篩選器並查看結果，但不得加入詢價籃或以企業客戶身分送出詢價。

## 4. 登入、導覽與登出

- B2C 登入頁提供 Email／密碼登入與 Google OAuth；新客可從 `/login` 前往 `/signup` 建立帳號。
- Email 登入模式提供「忘記密碼？」並導向 `/reset-password`；B2B 客戶代碼登入仍須驗證目前密碼後修改。
- Email 帳號由 Supabase Auth 建立；Google provider 與 redirect allow list 由 Supabase 專案設定。

| 帳號類型 | 登入成功後 | 已登入時直接開啟 `/login` |
|---|---|---|
| B2C 會員 | 導向 B2C 首頁 `/` | 導向 `/` |
| B2B 公司使用者 | 導向 `/business/catalog` | 導向 `/business/catalog` |
| 管理者 | 導向 `/admin` | 導向 `/admin` |
| 停用的 B2B 公司 | 不建立 session；留在 `/login` 並顯示停用提示 | 不適用 |

停用提示：

> 公司帳號已停用，請聯絡管理人員。

### 登入後導覽

| 角色 | 登入後導覽 |
|---|---|
| B2C 會員 | 會員中心、登出 |
| B2B 公司使用者 | 企業型錄、詢價籃／詢價紀錄、登出 |
| 管理者 | 管理後台、登出 |

所有已登入角色均不再顯示「登入」。所有角色登出後一律清除 session，並回到 B2C 首頁 `/`。

## 5. API 權限表

| API | 操作 | 未登入 | B2C 會員 | B2B 公司使用者 | 管理者 |
|---|---|---|---|---|---|
| `/api/b2c/mock-orders` | 建立展示用模擬訂單 | 允許 | 允許 | 拒絕 | 拒絕 |
| `/api/b2c/mock-orders` | 讀取模擬訂單 | 拒絕 | 拒絕 | 拒絕 | 允許 |
| `/api/b2c/mock-orders` | 更新訂單狀態 | 拒絕 | 拒絕 | 拒絕 | 允許 |

- 建立時由伺服器驗證收件人、手機、Email、配送地址與隱私權同意。
- 只能建立展示用模擬訂單，不得產生真實交易。
- 狀態限於已建立、處理中或已完成。
- 管理者透過 `/admin` 的 B2C 模擬訂單頁籤讀取及更新狀態。
- PRD 未指定 HTTP 方法；本文件只固定操作權限，不自行增加 API 端點。

## 6. 不建立獨立路由的功能

| 功能 | 呈現位置 |
|---|---|
| B2B 商品詳情 | `/business/catalog` 內的詳情狀態 |
| 結帳成功 | `/checkout` 的成功狀態 |
| 詢價成功 | `/business/rfq` 的成功狀態 |
| B2C 浮動需求工具 | 所有 B2C 頁面上的對話框 |
| B2B 過往詢價 | `/business/rfq` 的頁籤 |
| B2B 管理商品、篩選器與詢價 | `/admin/business` 的頁籤 |
| 其他管理後台模組 | `/admin` 的頁籤 |

## 7. 相較 PRD 的確認調整

- 新增獨立 B2C 分類路由 `/products/categories/[slug]`。
- 新增最小版 B2C 會員中心 `/user`。
- `/business` 是 B2B 內容首頁；新增 `/business/about`、`/business/news` 及其品牌故事、分類與文章詳情路由。
- `/news`、`/news/[slug]`、`/media` 與 `/media/[slug]` 為 B2C 公開核心內容，均可索引並加入 sitemap。
- `/business/homepage-preview`、`/business/prototype-home` 與 `/catalog-preview/*` 僅供預覽／原型使用，設定 `noindex` 且不加入 sitemap。
- 管理者使用統一 `/login` 與專屬帳密，登入頁不顯示管理員選項。
- 新增 `/admin/business`，集中處理 B2B 管理功能。
- `/business/rfq` 增加過往詢價紀錄頁籤。
- 管理者不能實際操作購物車或建立模擬訂單。
- B2B 誤入 B2C 商品、購物車或結帳路由時由伺服器阻擋並導向 `/business`。
- `/login`、`/cart`、`/checkout`、`/user` 均為 `noindex`，且不加入 sitemap。

> 本文件已涵蓋 PRD Sitemap、路由權限、登入分流、管理後台、B2B 詢價、B2C 結帳、SEO 索引及逐題確認結果，可作為後續開發、測試與驗收基準。
