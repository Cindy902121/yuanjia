...
- B2B 商品編輯頁、既有標籤與規格選項管理、圖片／Storage 管理、CSV 批量新增、批次狀態 API、staff 與 B2B tags API 已整合，並由 `business_staff` 與 `admin` 依權限使用。
...
| B2C 會員 | 使用 Email／密碼或 Google 帳號登入，可進入 `/user`。 |
...
| `/media/[slug]` | 媒體報導深度詳情 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/news` | 元家第一手最新消息 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
| `/news/[slug]` | 元家消息詳情 | 允許 | 允許 | 允許 | 允許 | 可索引／加入 |
...
| `/business/about` | B2B 品牌故事入口 | 導向 `/login` | 導向 `/` | 轉至 `/business/about/company` | 導向 `/admin` | `noindex`／不加入 |
| `/business/about/[slug]` | B2B 品牌故事內容 | 導向 `/login` | 導向 `/` | 允許 | 導向 `/admin` | `noindex`／不加入 |
| `/business/news` | B2B 企業消息入口 | 導向 `/login` | 導向 `/` | 轉至 `/business/news/activities` | 導向 `/admin` | `noindex`／不加入 |
| `/business/news/{activities,offers,yuanjia}` | B2B 企業消息分類 | 導向 `/login` | 導向 `/` | 允許 | 導向 `/admin` | `noindex`／不加入 |
| `/business/news/article/[slug]` | B2B 企業消息詳情 | 導向 `/login` | 導向 `/` | 允許 | 導向 `/admin` | `noindex`／不加入 |
...
> `/business/homepage-preview`、`/business/prototype-home` 與 `/catalog-preview/*` 是非 MVP 預覽／原型路徑，不納入本表的正式角色權限；僅保留直接預覽用途並設定 `noindex`，不加入 sitemap。
...
- `/business` 是登入後的 B2B 內容首頁，企業型錄仍由 `/business/catalog` 提供。
- `/business/about` 與 `/business/news` 是入口／分類導向頁；其子頁面只允許有效 B2B session 查看。
- B2B 內容頁不顯示價格、不加入 sitemap，並設定 `noindex`。
...
- P1 提供企業會員、B2C 商品、B2C 模擬訂單、B2B 詢價與 B2B 商品狀態管理；C 分支整合的商品內容、標籤、規格、圖片、CSV 與管理帳號能力亦由相應 API 提供。
- 提供前往 `/admin/business` 的入口。

### `/admin/business`

- B2B 商品清單支援搜尋、狀態篩選、合法批次狀態轉換、圖片數量與「尚無圖片」提示；編輯頁集中維護基本欄位、既有標籤、規格選項與圖片。
- 圖片限制為 1 張封面、最多 5 張細節圖、JPEG／PNG／WebP、單張 5 MB 以內且必填替代文字；B2B 圖片使用短效 signed URL。
- 管理者可操作篩選器並查看結果，但不得加入詢價籃或以企業客戶身分送出詢價。

## 4. 登入、導覽與登出

- B2C 登入頁提供 Email／密碼登入與 Google OAuth；新客可從 `/login` 前往 `/signup` 建立帳號。
- Email 帳號由 Supabase Auth 建立；若專案啟用 Email confirmation，完成註冊後須依信件連結驗證。
- Google OAuth 與 Email confirmation 的 provider／redirect allow list 設定由 Supabase 專案管理，不寫入前端或 repository secret。
...
- 新增最小版 B2C 會員中心 `/user`。
- `/business` 是 B2B 內容首頁；新增 `/business/about`、`/business/news` 及其品牌故事、分類與文章詳情路由。
- `/news`、`/news/[slug]`、`/media` 與 `/media/[slug]` 為 B2C 公開核心內容，均可索引並加入 sitemap。
- `/business/homepage-preview`、`/business/prototype-home` 與 `/catalog-preview/*` 僅供預覽／原型使用，設定 `noindex` 且不加入 sitemap。
- 管理者使用統一 `/login` 與專屬帳密，登入頁不顯示管理員選項。
- 新增 `/admin/business`，集中處理 B2B 管理功能。
- `/business/rfq` 增加過往詢價紀錄頁籤。
- 管理者不能實際操作購物車或建立模擬訂單。
- B2B 誤入 B2C 商品、購物車或結帳路由時由伺服器阻擋並導向 `/business`；`/api/b2c/products` 回傳 403。
