...
const { data: company, error: companyError } = await admin
  ...
  .single();
if (companyError || !company) throw companyError ?? new Error(`B2B company ${b2bIdentifier} is missing.`);
...
console.log(`Local contract identities ready: ${[b2c, b2b, adminUser, businessStaff].length} users.`);
