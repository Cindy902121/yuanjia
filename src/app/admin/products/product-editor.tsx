...
function ProductField({
  ...
  const isTextArea = isLongText || rule.key === "food_safety_info" || rule.key === "quality_info";
...
export function ProductEditor({
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (isEditing && !lastModified) {
      setSaveError("商品版本資訊遺失，請重新讀取後再儲存。");
      return;
    }
    setSaving(true);
    try {
      ...
      if (isEditing) {
        delete payload.slug;
        payload.expected_updated_at = lastModified;
        if (statusDirty) {
