...
/* eslint-disable @next/next/no-img-element -- Admin image URLs are generated by the server. */
...
async function requestJson<T>(input: RequestInfo | URL, init?: RequestInit) {
  ...
  if (!response.ok) {
    ...
    failure.storage_path = payload.storage_path ?? payload.storage_cleanup_path ?? undefined;
    throw failure;
  }
  return payload;
}
...
function requestFailure(error: unknown, fallback: string) {
  ...
  return {
    ...
    cleanupPath: failure.storage_cleanup === "failed" ? failure.storage_path ?? failure.storage_cleanup_path ?? null : null,
  };
}
...
export function ProductImageManager({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void requestJson<{ images: ProductImage[] }>(imageApiPath(channel, productId))
      .then((result) => {
        if (!cancelled) {
          setLoadError("");
  ...
  function updateLocalImage(imageId: string, update: Partial<ProductImage>) {
  ...
  async function uploadImage() {
    ...
    try {
      const result = await requestJson<{ image: ProductImage }>(
  ...
  async function saveAltText(image: ProductImage) {
    if (!productId || busy) {
      return;
    }
  ...
  async function replaceImage(imageId: string, file: File) {
    ...
    try {
      ...
      setPendingAltIds((current) => current.filter((id) => id !== image.id));
      if (result.storage_cleanup === "failed" && (result.storage_path ?? result.storage_cleanup_path)) {
        setCleanupWarning(result.storage_path ?? result.storage_cleanup_path ?? null);
  ...
  async function persistDetailOrder(nextDetails: ProductImage[]) {
    if (!productId || busy) {
      return;
    }
    ...
    setError("");
    try {
      const result = await requestJson<{ images: ProductImage[] }>(
        imageApiPath(channel, productId),
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  function renderImageCard(image: ProductImage, isCover: boolean, detailIndex?: number) {
