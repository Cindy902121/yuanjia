...
export default function ProductFinderClient() {
  ...
  const toggle = (key: string) => {
    const condition = B2B_FINDER_CONDITIONS[key as keyof typeof B2B_FINDER_CONDITIONS];
    if (!selected.includes(key)) {
      trackEvent({
        event_name: "b2b_product_finder_answer",
        event_data: { question_key: condition?.type ?? "condition", option_id: key },
      });
    }
