...
const productGroups = [
  { key: "fish", label: "魚類", en: "FISH", image: "/products/b2b/B2B-FISH-001/main.jpg", copy: "整魚、切片與餐飲規格，適合多元菜單與通路需求。", items: "鮭魚｜鱈魚｜鯖魚" },
...
function Reveal({ children, className = "", delay = 0 }: { children: ReactNode; className?: string; delay?: number }) {
  ...
  return <div className={`motion-safe:transition-[opacity,transform] motion-safe:duration-[900ms] motion-safe:ease-[cubic-bezier(0.22,1,0.36,1)] ${isVisible ? "motion-safe:translate-y-0 motion-safe:opacity-100" : "motion-safe:translate-y-8 motion-safe:opacity-0"} ${className}`} ref={ref} style={{ transitionDelay: `${delay}ms` }}>{children}</div>;
}

function SiteHeader() {
  return <header className="absolute inset-x-0 top-0 z-20 border-b border-white/20 bg-[#102C34]/35 text-white backdrop-blur-sm"><div className="mx-auto flex max-w-[1360px] items-center justify-between gap-6 px-6 py-5 lg:px-10"><div className="flex items-center gap-3"><Image alt="元家" height={42} src="/yens-logo.png" width={140} /><span className="h-7 w-px bg-white/40" /><span className="text-xs tracking-[0.22em] text-white/80">BUSINESS</span></div><nav className="hidden items-center gap-8 text-sm lg:flex"><a href="#products">元家產品</a><a href="#media">媒體報導</a><a href="#safety">食品安全</a><a href="#sustainability">永續責任</a></nav><a className="border border-white/70 px-4 py-2 text-sm font-semibold transition hover:bg-white hover:text-[#102C34]" href="/login">企業登入</a></div></header>;
}

export function Hero({ includeHeader = true }: { includeHeader?: boolean } = {}) {
  ...
  return (
    <section className="bg-white py-24" id="products">
      <div className="mx-auto max-w-[1360px] px-6 lg:px-10">
        <Reveal><div className="flex flex-wrap items-end justify-between gap-6"><div><p className="text-xs font-semibold tracking-[0.24em] text-[#00712F]">YUANJIA PRODUCTS</p><h2 className="mt-4 font-serif text-4xl text-[#17242A] sm:text-5xl">元家產品</h2></div><p className="max-w-md text-sm leading-7 text-[#536168]">依產品分類快速瀏覽，進入企業型錄查看完整規格與供應資訊。</p></div></Reveal>
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{productGroups.map((group, index) => <Reveal className="h-full" delay={index * 110} key={group.key}><a className="group relative block aspect-[.82/1] min-h-72 overflow-hidden bg-[#17353E] transition-transform duration-300 ease-out hover:-translate-y-1 focus-visible:-translate-y-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#005DAA]" href="/business/catalog"><Image alt={`${group.label}產品`} className="object-cover opacity-90 transition duration-500 group-hover:scale-105 group-hover:opacity-75" fill sizes="(min-width: 1024px) 25vw, (min-width: 640px) 50vw, 100vw" src={group.image} /><div className="absolute inset-0 bg-gradient-to-t from-[#102C34]/85 via-[#102C34]/10 to-transparent" /><div className="absolute inset-x-5 bottom-5 text-white"><p className="text-[10px] font-semibold tracking-[0.22em] text-[#C8E5EC]">{group.en}</p><h3 className="mt-2 text-2xl font-semibold">{group.label}</h3><p className="mt-2 text-sm text-white/80">查看企業供應分類 <span className="ml-1 inline-block transition-transform group-hover:translate-x-1">→</span></p></div></a></Reveal>)}</div>
        <Reveal delay={220}><div className="mt-10 text-center"><a className="inline-flex min-h-12 items-center border border-[#005DAA] px-7 text-sm font-semibold text-[#005DAA] transition hover:bg-[#005DAA] hover:text-white" href="/business/catalog">瀏覽完整企業型錄 →</a></div></Reveal>
      </div>
    </section>
  );
}
...
export { Products, Media, Safety, Sustainability, Footer };

export default function HomepagePreviewClient() { return <div className="min-h-screen bg-[#F7F6F2] font-sans text-[#17242A]"><div className="border-b border-[#E2B04A] bg-[#FFF7E1] px-4 py-2 text-center text-xs text-[#6F5112]">B2B 首頁概念預覽 · 尚未套用正式官網</div><Hero /><Products /><Media /><Safety /><Sustainability /><Footer /></div>; }
