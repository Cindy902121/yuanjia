...
export async function POST(request: Request) {
  ...
  const rowByProductCode = new Map<string, number>();
  for (const [index, row] of rows.entries()) {
    ...
    rowByProductCode.set(productCode, index + 2);
  ...
  try {
    ...
    const existingCode = existing?.[0]?.product_code;
    if (existingCode) {
      return apiError(
        `第 ${rowByProductCode.get(existingCode) ?? "未知"} 列的 product_code 已存在，整批未寫入。`,
        409,
      );
    }
