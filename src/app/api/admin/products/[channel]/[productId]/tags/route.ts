  ...
  try {
    ...
    const [{ data: product, error: productError }, { data: tags, error: tagError }, { data: relations, error: relationError }] =
      await Promise.all([
        ...
        admin.from(tagTable).select("id, group_name, slug, name").eq("is_active", true).order("group_name").order("name"),
        admin.from(relationTable).select("tag_id").eq("product_id", productId),
      ]);
    if (productError || tagError || relationError) return apiError("目前無法讀取產品標籤。", 503);
    if (!product) return apiError("找不到指定產品。", 404);
    return json({
      channel,
