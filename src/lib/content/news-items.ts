 ...
 *
 * 跟 src/lib/content/media-items.ts（媒體報導精選連結清單）不一樣：`/media`
 * 是「別人報導我們」的第三方新聞連結清單，這裡的 `/news` 是**元家自己撰寫、
 * 掛在自己網站上的原創編輯內容**——根據公開報導的真實事實（時間、地點、
 * 數據、引言）改寫、加深加廣背景脈絡與 FAQ，不是逐字轉貼原文，也不是憑空
 * 杜撰。每篇都保留完整的原始出處連結，方便查核。
 *
 * 內容產製守則（使用者 2026-08-25 提出的優化原則，之後新增文章都要遵守）：
 * 1. 真實性最高原則：核心事實（時間、地點、人物、數據、事件經過）必須跟
 *    原文完全一致，不能捏造假數據或虛構引言。
 * 2. 加深加廣僅限於：背景脈絡、產業影響、名詞解釋、FAQ、趨勢分析——不是
 *    新增原文沒有的「事實」。
 *
 * 第一篇文章（`2wan-dun-supply-chain`）改寫自今周刊 2025/10/08（第 1503
 * 期，記者徐采薇）原始報導，來源網址見 `sourceUrl`；同一篇報導也收錄在
 * media-items.ts 裡（第三方報導清單），兩邊資料來源相同，用途不同，不是
 * 重複維護。
 */
...
export const NEWS_ARTICLES: NewsArticle[] = [
  {
    ...
    title: "年掌握 2 萬噸海鮮貨源！元家企業如何成為台灣冷凍海鮮供應鏈龍頭",
